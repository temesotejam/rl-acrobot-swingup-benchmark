...
def comparison_row(algorithm: str, row: dict, train_time_s: float) -> str:
    return (
        ...
        f"<td>{pct(row['final_stable_rate'])}</td><td>{pct(row['goal_ratio'])}</td>"
        f"<td>{float(row['rms_torque_nm']):.3f} Nm</td><td>{train_time_s:.1f}s</td>"
        "</tr>"
    )
...
def build(root: Path, output: Path, run_url: str) -> None:
    ...
    for algorithm in ORDER:
        ...
        cards.append(
            ...
            f"difficulty {difficulty_text(best)} / Capture {pct(best['capture_rate'])}</p>"
    ...
    table_head = "<thead><tr><th>Algorithm</th><th>Checkpoint</th><th>Consumed steps</th><th>Difficulty</th><th>Capture</th><th>Capture time</th><th>Final stable</th><th>Goal height</th><th>RMS torque</th><th>Train time</th></tr></thead>"
...
<section class='hero'><h1>Continuous Acrobot Swing-up Benchmark</h1><p>同一の2リンク劣駆動系を PPO / SAC / TD3 で比較します。40秒episode、連続difficulty curriculum、best checkpoint rollbackを使用します。</p><p>Difficultyは0.000（upper寄り）から1.000（downwardを40%含む）まで連続的に変化し、悪化時には刻み幅も縮小します。</p><p><a href='{html.escape(run_url)}'>GitHub Actions run</a></p></section>
...
<section class='card'><h2>全評価チェックポイント</h2><div style='overflow:auto'><table><thead><tr><th>Algorithm</th><th>Stage</th><th>Consumed steps</th><th>Difficulty</th><th>Return</th><th>Capture</th><th>Capture time</th><th>Final stable</th><th>Goal height</th><th>RMS torque</th></tr></thead><tbody>{''.join(stage_rows)}</tbody></table></div></section>
<section class='card'><h2>評価条件</h2><p>全方式で同じ物理、reward、センサノイズ、評価seedを使用。Captureは先端高さ1.0 m以上かつ低角速度を0.5秒維持、Final stableは最終2秒の80%以上で同条件を満たすことです。連続difficultyはdownward評価が十分なら少しずつ上げ、大きく悪化した場合はbest checkpointへ戻して次の刻みを細かくします。</p></section>
</main></body></html>"""
