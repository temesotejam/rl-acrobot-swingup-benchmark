...
def comparison_row(algorithm: str, row: dict, train_time_s: float) -> str:
    return (
        ...
        f"<td>{int(float(row['timesteps'])):,}</td><td>{pct(row['capture_rate'])}</td>"
...
def build(root: Path, output: Path, run_url: str) -> None:
    ...
    for algorithm in ORDER:
        ...
        for row in rows:
            stage_rows.append(
                ...
                f"<td>{capture_time(row['mean_capture_time_s'])}</td><td>{pct(row['final_stable_rate'])}</td>"
                f"<td>{pct(row['goal_ratio'])}</td><td>{float(row['rms_torque_nm']):.3f}</td>"
                "</tr>"
            )
        cards.append(
            ...
            f"<p><strong>{html.escape(str(best['stage']))}</strong> / {int(float(best['timesteps'])):,} steps / Capture {pct(best['capture_rate'])}</p>"
            f"<h3>Final training state</h3><video controls muted loop playsinline src='{algorithm}/videos/final.mp4'></video>"
    ...
    table_head = "<thead><tr><th>Algorithm</th><th>Checkpoint</th><th>Consumed steps</th><th>Capture</th><th>Capture time</th><th>Final stable</th><th>Goal height</th><th>RMS torque</th><th>Train time</th></tr></thead>"
...
video,img{{width:100%;border-radius:10px;background:#000}}table{{width:100%;border-collapse:collapse;font-size:14px}}th,td{{padding:8px;border-bottom:1px solid #30363d;text-align:right}}th:first-child,td:first-child{{text-align:left}}a{{color:#58a6ff}}code{{background:#21262d;padding:2px 5px;border-radius:5px}}h3{{margin-bottom:8px}}
</style></head><body><main>
<section class='hero'><h1>Continuous Acrobot Swing-up Benchmark</h1><p>同一の2リンク劣駆動系を PPO / SAC / TD3 で比較します。40秒episode、評価駆動adaptive curriculum、best checkpoint rollbackを使用します。</p><p><a href='{html.escape(run_url)}'>GitHub Actions run</a></p></section>
...
<section class='card'><h2>学習終了時の状態</h2><div style='overflow:auto'><table>{table_head}<tbody>{''.join(final_rows)}</tbody></table></div></section>
<section class='grid'>{''.join(cards)}</section>
<section class='card'><h2>全評価チェックポイント</h2><div style='overflow:auto'><table><thead><tr><th>Algorithm</th><th>Stage</th><th>Consumed steps</th><th>Return</th><th>Capture</th><th>Capture time</th><th>Final stable</th><th>Goal height</th><th>RMS torque</th></tr></thead><tbody>{''.join(stage_rows)}</tbody></table></div></section>
<section class='card'><h2>評価条件</h2><p>全方式で同じ物理、reward、センサノイズ、評価seedを使用。Captureは先端高さ1.0 m以上かつ低角速度を0.5秒維持、Final stableは最終2秒の80%以上で同条件を満たすことです。adaptive curriculumはdownward評価が改善すれば難しくし、大きく悪化した場合はbest checkpointへロールバックします。</p></section>
</main></body></html>"""
